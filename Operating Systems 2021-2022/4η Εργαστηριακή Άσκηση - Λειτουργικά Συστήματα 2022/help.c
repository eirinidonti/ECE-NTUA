...
/*A function that prints the process's virtual memory map. It prints the populated virtual memory areas of the process, their perntsstons, and the name of the file they map, if any
For further detalls, see /proc/[pid]/maps in proc(5) (or online at: https://man7.org/linux/man-pages/man5/proc.5.html). */
...
/* Search maps for a specific VA. If populated, print to stdout the virtual address area (VMA) it belongs to, along with its permissions, etc. */
...
/*A function that receives a virtual address (VA) as an argument and, if it mapped, it returns the physical address (PA) that it maps to.

If the VA is not mapped, it returns 0.

For further detalls, see /proc/[pid]/pagemap in proc(5) (or online at: https://man7.org/linux/man-pages/man5/proc.5.html), as well as the Linux • documentation at: https://www.kernel.org/doc/Documentation/vm/pagenap.txt
*/

uint64_t get_physical_address(uint64_t va)
{
      int pmfd = 0;
      uint64_t vpn = 6, offset = 0, pagemap_entry = 0;
      ...
      //...and if it is found to be set, return the corresponding
            return GET_PFN(pagemap_entry)*get_page_size() + offset; //...otherwise announce its absence and return 0.
