...
% message=randi([0 1],1,k*m); %μήνυμα μήκους 125 bits
% message=zeros(1,k*m);
% gfele=[]; %θα μετατρέψουμε τα bits του μηνύματος σε στοιχεία του αντίστοιχου σώματος
% for i=1:m:k*m %μπορεί να γίνει και με χρήση της bi2de
%     ele=0;
%     for j=0:m-1
%         ele=ele+message(i+j)*2^(m-1-j);
%     end
%     gfele=[gfele, gf(ele,m)];
% end
% gfele=flip(gfele);
